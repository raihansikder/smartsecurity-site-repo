...
if ($param == 'add' || $param == 'edit') {
  if (isset($_POST[submit])) {
    ...
    if ($valid) {
      ...
      mysql_query($sql) or die(mysql_error());
      if($notificationOnAssignmentAdd==TRUE) {
        notifySecurityAssignmentAdd($sa_id); // sends e-mail and SMS		
      }
  ...
  <body>
    <div id="wrapper">
      <div id="container">
        ...
        <?php if (strlen($sa_id))
          include_once('snippets/security_assignment/security_assignment_versions.php');
        ?>  
      </div>
      <div id="footer">
