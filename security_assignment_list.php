	...
	<div id="wrapper">
		<div id="container">
					...
					<div class="quickadd" style="background-color: #EAEBFF; padding: 10px; border-radius: 5px 5px 5px 5px;">
						<form action="security_assignment_add.php" method="post" enctype="multipart/form-data">
						  ...
						  <table width="100%" border="0" align="left" cellpadding="0" cellspacing="0" id="list">
								<tr>
									<td align="left" valign="top">Company/Client: <br> <?php
									...
									?> &nbsp; <a href=" client_add.php?param=add"> <img src="images/plus-small.png" alt="Add new Company/client">

									</a>
									</td>
									<td align="left" valign="top">Requested by : <br> <?php
									...
									?> &nbsp; <a href="user_add.php?param=add&user_type_id=2"> <img src="images/plus-small.png" alt="Add new requeester/contact">

									</a>
									</td>
									<td align="left" valign="top">Site: <br> <?php
									...
									?> &nbsp; <a href="security_site_add.php?param=add"> <img src="images/plus-small.png" alt="Add new Site">

									</a>
									</td>
									<td align="left" valign="top">Guard name: <br> <?php
									...
									?> &nbsp; <a href="user_add.php?param=add&user_type_id=3"> <img src="images/plus-small.png" alt="Add new Guard">

									</a>
									</td>
								</tr>
								<tr>
								  <td align="left" valign="top">Start date time: <br> <script>
                                                ...
                                                </script> <input name="sa_start_time" type="text" value="<?php echo addEditInputField('sa_start_time'); ?>" size="20" class="validate[required]" readonly="readonly" />
									</td>
									<td align="left" valign="top">End date time: <br> <script>
                                                ...
                                                </script> <input name="sa_end_time" type="text" value="<?php echo addEditInputField('sa_end_time'); ?>" size="20" class="validate[required]" readonly="readonly" />
									</td>
									<td align="left" valign="top">Rate (per hour): <br> <input name="sa_rate" type="text" value="<?php echo addEditInputField('sa_rate'); ?>" size="20" class="validate[required,custom[number]]" />
									</td>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" id="datatable_nopagination" style="text-shadow: white 0.1em 0 0">
						<thead style="font-weight:bold;">
							<tr>								
                                ...
                                <td align="right"><?php if (hasPermission('security_assignment', 'delete', $_SESSION[current_user_id])) { ?> Delete<?php } ?></td>
			...
			for ($i = 0; $i < $rows; $i++) { ?>
							...
							?>> 
								<td><span class="small" style="font-size:0px;"> <?php echo date('ymd', strtotime($arr[$i][sa_start_time])); ?>
								</span> <br><b>
								<?php echo date('d-M-y', strtotime($arr[$i][sa_start_time])); ?></b>
								</td>
                                <td><?php echo "W" . date("W", strtotime($arr[$i][sa_start_time])) . "";	?>
								</td>
								<td><?php echo date('l', strtotime($arr[$i][sa_start_time])); ?>
								</td>
								<td class="greenText"><?php echo date('H:i', strtotime($arr[$i][sa_start_time])); ?>
								</td>
								<td class="orangeText"><?php echo date('H:i', strtotime($arr[$i][sa_end_time])); ?>
								</td>
                                <td><?php $hour= round(my_hour_diff($arr[$i][sa_start_time], $arr[$i][sa_end_time]), 2);
								$total_hours+=$hour;
								echo  $hour; ?></td>
								<td><?php echo getUserFullNameFrmId($arr[$i][sa_requested_by_user_id]); ?>
								</td>
								<td><?php echo getClientCompanyNameFrmId($arr[$i][sa_client_id]); ?>
								</td>
								<td><?php echo getSiteNameFrmId($arr[$i][sa_site_id]); ?>
								</td>
								<td><?php echo getUserFullNameFrmId($arr[$i][sa_guard_user_id]); ?>
								</td>
                                <?php 
								if(currentUserIsGuard()){
								?>
								<td><?php echo $arr[$i][sa_rate]; ?>
								</td>								
								<td><?php $amount = round(my_hour_diff($arr[$i][sa_start_time], $arr[$i][sa_end_time]) * $arr[$i][sa_rate], 2);
								$total_amount+=$amount;
								echo $amount; ?></td>
                                <?php } ?>
								<td><?php echo nl2br($arr[$i][sa_comment]); ?>
								</td>
								<td><span style="width: 65px; float: right;"> <?php if (hasPermission('security_assignment', 'add', $_SESSION[current_user_id])) { ?> <a href="security_assignment_add.php?sa_id=<?php echo $arr[$i][sa_id]; ?>&param=edit" class="none">Edit</a> <?php } ?> <a href="security_assignment_add.php?sa_id=<?php echo $arr[$i][sa_id]; ?>&param=view" id="<?php echo $arr[$i][sa_id]; ?>"> <img src="images/viewIcon.png" alt="View" title="View" width="15" height="15" />
									</a> 
                                    ...
                                    <span id="ajax_working_<?=$arr[$i][sa_id]?>"></span>
								</span>
                                <script>
				...
				<div id="mid_right">
					...
					<form action="" method="get">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<?php if (hasPermission('security_assignment', 'delete', $_SESSION[current_user_id])) { ?>
							<tr>
								<td width="15%">Company/Client <br> <?php
								...
								<tr>
									<td>Week Number: <br> <input name="sa_week_number" type="text" value="<?php echo addEditInputField('sa_week_number'); ?>" size="20" class="validate[min[1], max[54], custom[number]]" style="float: none;" />
									</td>
								</tr>
								<tr>
									<td>Month: <br /> <?php createDropdown($month_arr, 'sa_month'); ?> <!--<input name="sa_month" type="text" value="<?php echo addEditInputField('sa_month'); ?>" size="20" class="validate[min[1], max[54], custom[number]]" style="float:none;" />-->
									</td>
								</tr>
								<tr>
									<td>Year: <br> <input name="sa_year" type="text" value="<?php echo addEditInputField('sa_year'); ?>" size="20" class="validate[min[2012], max[9999], custom[number]]" style="float: none;" placeholder="2012" />
									</td>
								</tr>
								<?php if (hasPermission('security_assignment', 'delete', $_SESSION[current_user_id])) { ?>
								<tr>
									<td>Requested by <br> <?php
								...
								<?php } ?>
								<tr>
									<td><input type="submit" name="submit" value="Filter" class="bgblue button" /> <a href="security_assignment_list.php" class='button bgblue'>Reset</a>
									</td>
								</tr>
